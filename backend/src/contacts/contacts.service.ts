                ...
                switch (column.type) {
                    case ColumnType.NUMBER: {
                        ...
                        if (Number.isNaN(numericValue)) {
                            // Incomplete/invalid number input (e.g. user still typing) -> no matches, don't crash
                            qb.andWhere('1 = 0');
                        } else {
                    ...
                    case ColumnType.DATE: {
                        const isValidDate = /^\d{4}-\d{2}-\d{2}$/.test(filterValue);
                        if (!isValidDate) {
                            // Incomplete/invalid date input -> no matches, don't crash
                            qb.andWhere('1 = 0');
                        } else {
